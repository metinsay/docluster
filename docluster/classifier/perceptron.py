class Perceptron(object):
    ...
    def train(self, data):
        ...
        if self.kind == 'standard':
            ...
            for _ in range(self.n_iterations):
                for feature, label in zip(data, data.index):
            ...
            for _ in range(self.n_iterations):
                for feature_vector, label in zip(data, data.index):
                    (weights, offset) = self.update_weights(feature_vector, label, weights, offset)
                    sum_theta = np.add(sum_theta, weights)
                    sum_offset += offset

            self.weights, self.offset = sum_theta / (n_data * self.n_iterations), sum_offset / (n_data * self.n_iterations)
    ...
    def fit(self, data):
        n_data = data.shape[0]
        tiled_weight = np.tile(self.weights, (n_data, 1))
        evaled_data = np.einsum('ij,ij->i', tiled_weight, self.weights) + offset
        return evaled_data <= 0
